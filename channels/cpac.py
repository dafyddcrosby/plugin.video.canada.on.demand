...
class CPAC(BaseChannel):
    ...
    def action_play_video(self):
        ...
        soup = BeautifulSoup(
            self.plugin.fetch(remote_url, max_age=self.cache_timeout))
        obj = soup.find("object", {'id': "MPlayer2"})
        vidurl = obj.find('param', {'name': 'url'})['value']
        asx = BeautifulSoup(
            self.plugin.fetch(vidurl, max_age=self.cache_timeout))
    ...
    def action_list_episodes(self):
        soup = BeautifulSoup(self.plugin.fetch(self.base_url +
                                               self.args['remote_url'], max_age=self.cache_timeout))
    ...
    def action_list_shows(self):
        soup = BeautifulSoup(
            self.plugin.fetch(self.base_url, max_age=self.cache_timeout))
