...
class TVOKids(BrightcoveBaseChannel):
    ...
    def get_swf_url(self):
        ...
        location = base.replace(
            "BrightcoveBootloader.swf", "federatedVideo/BrightcovePlayer.swf")
    ...
    def action_play_video(self):
        ...
        qs = "?videoId=%s&lineUpId=&pubId=%s&playerId=%s&affiliateId=" % (
            self.video_id, self.publisher_id, self.player_id)
        #playpath += "&" + wierdqs
        ...
        url = "%s tcUrl=%s app=%s playpath=%s%s swfUrl=%s conn=B:0 conn=S:%s&%s" % (
            tcurl, tcurl, app, playpath, qs, self.swf_url, playpath, wierdqs)
        logging.debug(url)
        self.plugin.set_stream_url(url)

    def action_browse_show(self):
        url = self.base_url + "/feeds/%s/all/videos_list.xml?random=%s" % (
            self.args['node_id'], int(time.time()), )
        ...
        for node in soup.findAll('node'):
            ...
            data['Title'] = decode_htmlentities(
                node.find('node_title').contents[0].strip())
            data['Plot'] = decode_htmlentities(
                node.find("node_short_description").contents[0].strip())
    ...
    def action_list_shows(self):
        ...
        page = self.plugin.fetch(
            self.base_url + url, max_age=self.cache_timeout).read()
        soup = BeautifulStoneSoup(page)
        for node in soup.findAll('node'):
            data = {}
            data.update(self.args)
