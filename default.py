...
class OnDemandPlugin(object):
    ...
    def connect_to_db(self):
        path = xbmc.translatePath(
            'special://profile/addon_data/plugin.video.canada.on.demand/')
    ...
    def action_channel_list(self):
        ...
        for channel_code, channel_class in sorted(ChannelMetaClass.registry.channels.iteritems()):
            ...
            try:
                info['Thumb'] = self.get_resource_path(
                    'images', 'channels', info['Thumb'])
            except ChannelException:
                logging.warn(
                    "Couldn't Find Channel Icon for %s" % (channel_code,))
    ...
    def add_list_item(self, info, is_folder=True, return_only=False,
        ...
        if bookmark_parent is None:
            bookmark_url = self.get_url(
                {'action': 'add_to_bookmarks', 'url': self.get_url(info)})
            context_menu_items.append(
                ("Bookmark", "XBMC.RunPlugin(%s)" % (bookmark_url,)))
        else:
            bminfo = {'action': 'remove_from_bookmarks', 'url':
                      self.get_url(info), 'folder_id': bookmark_parent}
            ...
            context_menu_items.append(("Remove From Bookmarks",
                                       "XBMC.RunPlugin(%s)" % (bookmark_url,)))
        ...
        li.setInfo(type='Video', infoLabels=dict((k, unicode(v))
    ...
    def add_bookmark_folder(self):
        ...
        if val == -1:
            return None
        parent = rows[val]
        name = self.get_modal_keyboard_input(
            'New Folder', 'Enter the name for the new folder')
        ...
        if curs.fetchall():
            dialog.ok("Failed!", "Couldn't create folder: %s because it already exists" %
                      (newpath,))
            return None
    ...
    def action_browse_bookmarks(self):
        ...
        if not bookmarks:
            ...
            for bm in bookmarks:
                ...
                data['Title'] = bm[1]
                self.add_list_item(data, is_folder=True,
                                   bookmark_parent=bm[3], bookmark_id=bm[0])
    ...
    def remove_folder_from_bookmarks(self, parent_id, folder_id):
        ...
        if dialog.yesno("Are you Sure?", "Are you sure you wish to delete the bookmark folder: %s\n(All Bookmarks and Folders within it will be deleted!)" % (record[3],)):
            ...
            curs.execute("select id from bookmark_folders where path like ?",
                         (record[3] + "%",))
            rows = curs.fetchall()
            for row in rows:
    ...
    def remove_bookmark_from_bookmarks(self, parent_id, bookmark_id):
        ...
        if dialog.yesno("Are you Sure?", "Are you sure you wish to delete the bookmark: %s" % (record[1],)):
            ...
            curs.execute("delete from bookmarks where folder_id = ? and id = ?",
